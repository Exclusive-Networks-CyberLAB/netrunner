...
![A screenshot of the NetRunner OS application, showing the three tabs and cyberpunk UI.](./images/interface.png)

NetRunner OS is a self-contained, single-file Python web application designed for network security professionals, NDR (Network Detection and Response) testers, and hobbyists.

The application combines multiple core tools into a single, unified interface:

1.  **A PCAP Replayer:** To replay existing packet captures (PCAPs) with complex, on-the-fly rewriting rules.
2.  **A Traffic Generator:** To craft and send stateless TCP, UDP, or ICMP packets from scratch.
3.  **An Intelligent Packet Viewer:** To analyze an uploaded PCAP, map IP to MAC addresses, and heuristically identify potential attacker IP.
...
* **Configuration state** save your configuration for quick retrieval of IP and MAC address mappings, loop interations and timing
...
1.  **Clone the repository (or download the script):**
    ...
    ```bash
    git clone (https://github.com/Exclusive-Networks-CyberLAB/netrunner.git)
    cd netrunner
    ```

    *...or just save the `netrunner-v1.py` file to a new directory.*
...
2.  **Run the script with administrator privileges:**
    ...
    **On macOS / Linux:**
    ...
    ```bash
    sudo python3 networkrunner-v1.py
    ```
    ...
    **On Windows:**
    ...
    ```bash
    python networkrunner-v1.py
    ```

3.  **Automatic Setup:** The script will first check if `Flask` and `Scapy` are installed. If not, it will attempt to install them using `pip`.

4.  **Access the Application:** Once the script is running, it will print the local URL. Open your web browser and navigate to:
    **`http://127.0.0.1:9000`**
...
1.  Enter a **Name**, **IP**, and **MAC** for a device you want to save (e.g., "Test Victim 1", "10.1.1.50", "00:11:22:33:44:55").
2.  Click **Add Asset**.
3.  This asset will now be available in dropdowns throughout the app to auto-fill fields. This list is in-memory and will be cleared when the app is restarted.

### 2\. PCAP Replayer & Viewer

The Replayer and Viewer tabs work together.

1.  **Upload PCAP:** On the **// PCAP Replayer //** tab, click "Choose File" and select a `.pcap` or `.pcapng` file.
2.  **Analyze (Automatic):** The app will immediately show an "Analyzing PCAP..." status. Once complete:
...
3.  **Review Traffic:** Click the **// Packet Viewer //** tab. You will see a table summarizing the first 500 packets. Use this to confirm the analysis and see the conversations.
...
6.  **Initiate:** Click the **Initiate Replay** button.
...
4.  **Generate:** Click the **Generate Traffic** button.
