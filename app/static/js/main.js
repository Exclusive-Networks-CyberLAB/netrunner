...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Sortable statistics tables (Wireshark-style) ──────────────────────
    // Default to the most useful view: endpoints by busiest (Tx packets),
    // conversations by longest duration.
    let endpointSort = { key: 'tx_pkts', dir: 'desc' };
    ...
    function renderEndpoints(endpoints) {
        ...
        body.innerHTML = '';
        sortRows(endpoints, endpointSort.key, endpointSort.dir).forEach(ep => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td>${ep.ip}</td>
                <td>${ep.mac}</td>
    ...
    function renderConversations(conversations) {
        ...
        // Identify the longest conversation by duration, independent of sort.
        let longest = null;
        conversations.forEach(c => {
            if (!longest || Number(c.duration) > Number(longest.duration)) longest = c;
        });
        ...
        sortRows(conversations, conversationSort.key, conversationSort.dir).forEach(conv => {
            const tr = document.createElement('tr');
            const isLongest = longest && conv === longest && Number(conv.duration) > 0;
            if (isLongest) tr.classList.add('longest-convo');
    ...
    // Wire up click-to-sort on the statistics tables. Re-render from the stored
    // analysis result so sorting persists without re-fetching.
    makeSortable('endpointsTable', endpointSort, () => {
        if (analysisData) renderEndpoints(analysisData.endpoints);
    });
    makeSortable('conversationsTable', conversationSort, () => {
        if (analysisData) renderConversations(analysisData.conversations);
    });
